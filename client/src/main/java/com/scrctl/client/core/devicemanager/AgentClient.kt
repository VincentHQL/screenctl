...
/**
 * 与设备上运行的 scrcpy-monitor HTTP 服务通信的客户端。
 *
 * 通过 [LocalSocketForwarder] 将设备上的 `localabstract:scrcpy_monitor`
 * 映射到客户端本地的 abstract socket 后，AgentClient 通过自定义
 * [SocketFactory] 将 OkHttp 的连接重定向到该 abstract socket，
 * 从而以 HTTP 协议与设备通信。
 *
 * @param socketName 客户端本地的 abstract socket 名称
 */
class AgentClient(
    ...
    /**
     * 使用 abstract socket 作为传输层的 [SocketFactory]。
     *
     * OkHttp 会调用 [createSocket] 来建立 TCP 连接，这里将其替换为
     * [LocalSocket] 连接到指定的 abstract socket 名称。
     */
